...
namespace Pointer {
  ...
  public:
    ...
    /** Bloque la ressource.
     * Cette fonction est bloquante, c'est a dire que si la ressource est bloqué
par une autre ressource, on attend jusqu'à ce qu'elle soit libre.
     */
    void lock() {
     ...
     * Elle renvoit : EBUSY (man phtread_mutex_trylock) si le mutex est
verrouille, 0 sinon
     */
    int tryLock() {
    ...
    /** Debloque le mutex.
     * Renvoit EPERM (man phtread_mutex_unlock) si le mutex n'a pas été
verrouillé par la thread appelante, renvoit 0 sinon
     */
    int unlock() {
